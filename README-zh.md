...
| Skill | 命令 | 做什么 |
|-------|------|--------|
| **lgtm** | `/lgtm` | 创建验收清单，检查完成状态，产出验证报告 |
| **product-manager** | `/product-manager` | 做优先级决策，写 PRD，推进路线图 |
| **rtfd** | `/rtfd` | 从代码生成参考文档，从资料构建知识 wiki |
...
**Looks Good To Me** — 工程师 code review 时的经典审批用语，这里用来做验收。
...
`/lgtm` 在任务开始时创建**可验证的清单**，在 agent 完成时检查——像一个异步的 QA 线程跑在你的工作旁边。

**1. 定义验收标准**（首次 `/lgtm`，或让 agent 自动生成）：
...
**2. 检查进度**（后续 `/lgtm` 调用——agent 完成工作后可自动触发）：
...
**为什么叫 "lgtm"？** 因为目标就是在你 ship 之前，给你一个有证据支撑的"looks good to me"——而不是靠感觉。
...
**为什么叫 "product-manager"？** 因为这就是岗位名。这个 skill 不是帮你做 PM——它做 PM 的活。
...
运行 `/rtfd`，告诉它你要什么。Agent 自己判断该生成代码文档还是编译知识 wiki。
...
**为什么叫 "rtfd"？** 因为当有人问"文档在哪"的时候，答案应该是"跑一下 `/rtfd`"——而不是"以后再说"。
