...
`/verify` creates a **verifiable checklist** at the start of a task, then checks it when the agent finishes — like an async QA thread running alongside your work.

**1. Define acceptance criteria** (first `/verify` call, or auto-generated by agent):
...
**2. Check progress** (subsequent `/verify` calls — agent can run this automatically after completing work):
...
**Why "verify"?** Because the whole point is to give you a confident "looks good to me" before you ship — backed by evidence, not vibes.
...
**Why "pm"?** It's the job title. This skill doesn't help you be a PM — it does the PM job.
